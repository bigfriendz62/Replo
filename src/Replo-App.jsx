...
const FLUTTERWAVE_MONTHLY_LINK = "https://flutterwave.com/pay/zvvybftadmhl";
const FLUTTERWAVE_ANNUAL_LINK = "https://flutterwave.com/pay/39jqo8vckbmx";
...
// ─── Sections ─────────────────────────────────────────────────────────────────
const SECTIONS = [
  { id: "dashboard", icon: "▣", label: "Dashboard",        desc: "Your business overview" },
  ...
  { id: "history",  icon: "▥",  label: "Reply History",    desc: "Your past AI replies" },
...
// ─── Branding Component (Powered by Replo for free users) ─────────────────────
function PoweredByBadge({ isPro }) {
  if (isPro) return null;
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px 0", marginTop: 8 }}>
      <div style={{ width: 14, height: 14, borderRadius: "50%", background: "#4AE54A", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, fontWeight: 900, color: "#0a0f0a" }}>R</div>
      <span style={{ fontSize: 11, color: "#445544", letterSpacing: "0.04em" }}>Powered by Replo AI</span>
    </div>
  );
}
...
function PageSpinner() {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "60px 0" }}>
      <div style={{ width: 32, height: 32, border: "3px solid #1a2f1a", borderTopColor: "#4AE54A", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
    </div>
  );
}
...
function OnboardingModal({ onDone }) {
  ...
  const steps = [
    {
      ...
      sub: "Your first name is fine",
      field: <input value={ownerName} onChange={e => setOwnerName(e.target.value)} placeholder="E.g. Goodluck" style={inputStyle} />,
      valid: ownerName.trim().length > 1,
    },
    {
      title: "What's your business called?",
      sub: "Your shop, brand, or business name. If you don't have one yet, just type what people know you as.",
      field: <input value={bizName} onChange={e => setBizName(e.target.value)} placeholder="E.g. Tope's Fashion Store" style={inputStyle} />,
      valid: bizName.trim().length > 1,
    },
    {
  ...
  const finish = () => {
    const profile = { ownerName, bizName, biz, where, country, priceRange, delivery, payment };
...
function PaywallModal({ onClose, onSuccess }) {
  ...
  const paymentLink = plan === "monthly" ? FLUTTERWAVE_MONTHLY_LINK : FLUTTERWAVE_ANNUAL_LINK;
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 400, padding: 20 }}>
      <div style={{ background: "#0d150d", border: "1px solid #1a3f1a", borderRadius: 20, padding: "32px 28px", maxWidth: 440, width: "100%", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ background: "#111811", borderRadius: 12, padding: "16px", marginBottom: 18, border: "1px solid #1a2f1a" }}>
          {["Unlimited AI replies daily", "Daily content generator", "Business advisor chat", "All message templates", "Startup business guide", "Marketing planner", "Weekly health check", "Invoice generator", "Priority responses"].map((f, i) => (
            <div key={i} style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: i < 8 ? 8 : 0 }}>
              ...
              <span style={{ fontSize: 13, color: "#aabbaa" }}>{f}</span>
            </div>
          ))}
        </div>

        <button onClick={pay} style={{ width: "100%", padding: "14px", background: "#4AE54A", color: "#0a0f0a", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 800, marginBottom: 10 }}>
          Pay {label} →
        </button>

        <button onClick={unlockNow} style={{ width: "100%", padding: "12px", background: "none", color: "#556655", border: "1px solid #1a2f1a", borderRadius: 10, fontSize: 13, marginBottom: 10 }}>
          Already paid? Tap here to unlock
        </button>
        ...
        <p style={{ textAlign: "center", fontSize: 11, color: "#334433", marginTop: 6, fontFamily: "Georgia, serif" }}>Secured by Flutterwave. Cancel anytime.</p>
      </div>
    </div>
  );
}
...
function SmartReply({ profile, isPro, onUpgrade }) {
  ...
  const saveToHistory = (customerMessage, aiReply) => {
    const history = store.get("replo_reply_history", []);
    const record = { customerMessage, aiReply, timestamp: Date.now() };
    const updated = [record, ...history].slice(0, 100);
    store.set("replo_reply_history", updated);
  };
...
Generate ONE professional, warm, and persuasive reply to the customer message. Keep it conversational but professional. Under 100 words. Do not include any preamble or explanation — just the reply text itself. Make it feel human, not robotic.`;
      ...
      setReply(text);
      saveToHistory(customerMsg, text);
...
// ─── Dashboard ──────────────────────────────────────────────────────────────
function Dashboard({ profile, isPro, onNavigate, onEditBusiness }) {
  const repliesUsed = store.get("replo_reply_count", 0);
  const history = store.get("replo_reply_history", []);
  const todayReplies = history.filter(h => {
  ...
  const quickActions = [
    { id: "reply",     icon: "⚡", label: "New Reply",       color: "#4AE54A" },
    { id: "content",   icon: "✦",  label: "Today's Content", color: "#4AE54A" },
    { id: "advisor",   icon: "◈",  label: "Ask Advisor",     color: "#4AE54A" },
    { id: "invoice",   icon: "▤",  label: "New Invoice",     color: "#4AE54A" },
  ];
  ...
  return (
    <div style={sectionWrap}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
        <div>
          <div style={{ fontSize: 11, color: "#556655", letterSpacing: "0.15em", textTransform: "uppercase", fontFamily: "monospace", marginBottom: 6 }}>Welcome back{profile.ownerName ? `, ${profile.ownerName}` : ""}</div>
          <h2 style={{ ...sectionTitle, marginBottom: 4 }}>{profile.bizName || "Your Business"}</h2>
        </div>
        {isPro && <div style={{ fontSize: 10, color: "#4AE54A", background: "rgba(74,229,74,0.1)", padding: "5px 10px", borderRadius: 6, fontFamily: "monospace" }}>PRO</div>}
      </div>
      <p style={sectionSub}>{profile.country} · {profile.where}</p>

      {/* Stats */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 24 }}>
        {[
          { value: todayReplies, label: "Replies today" },
          { value: repliesUsed, label: "Total replies" },
          { value: history.length, label: "In history" },
        ].map((s, i) => (
          <div key={i} style={{ background: "#0d150d", border: "1px solid #1a2f1a", borderRadius: 12, padding: "14px", textAlign: "center" }}>
            <div style={{ fontSize: 22, fontWeight: 800, color: "#4AE54A", marginBottom: 2 }}>{s.value}</div>
            <div style={{ fontSize: 10, color: "#556655" }}>{s.label}</div>
          </div>
        ))}
      </div>

      {/* Quick actions */}
      <div style={{ fontSize: 11, color: "#556655", letterSpacing: "0.12em", textTransform: "uppercase", fontFamily: "monospace", marginBottom: 12 }}>Quick Actions</div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 24 }}>
        {quickActions.map(a => (
          <div key={a.id} onClick={() => onNavigate(a.id)} style={{ background: "#0d150d", border: "1px solid #1a2f1a", borderRadius: 12, padding: "16px", cursor: "pointer", transition: "border-color 0.2s" }}
            onMouseEnter={e => e.currentTarget.style.borderColor = "#4AE54A"}
            onMouseLeave={e => e.currentTarget.style.borderColor = "#1a2f1a"}>
            <div style={{ fontSize: 22, color: a.color, marginBottom: 8 }}>{a.icon}</div>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#e8f5e8" }}>{a.label}</div>
          </div>
        ))}
      </div>

      {/* Business profile card */}
      <div style={{ fontSize: 11, color: "#556655", letterSpacing: "0.12em", textTransform: "uppercase", fontFamily: "monospace", marginBottom: 12 }}>My Business</div>
      <div style={{ background: "#0d150d", border: "1px solid #1a2f1a", borderRadius: 12, padding: "18px" }}>
        {[
          { label: "Your name", value: profile.ownerName },
          { label: "Business name", value: profile.bizName },
          { label: "Sells", value: profile.biz },
          { label: "Platforms", value: profile.where },
          { label: "Country", value: profile.country },
          { label: "Price range", value: profile.priceRange },
          { label: "Delivery", value: profile.delivery },
          { label: "Payment methods", value: profile.payment },
        ].map((f, i, arr) => (
          <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: i < arr.length - 1 ? "1px solid #1a2f1a" : "none" }}>
            <span style={{ fontSize: 13, color: "#556655" }}>{f.label}</span>
            <span style={{ fontSize: 13, color: "#e8f5e8", textAlign: "right", maxWidth: "60%" }}>{f.value || "—"}</span>
          </div>
        ))}
        <button onClick={onEditBusiness} style={{ width: "100%", marginTop: 14, padding: "11px", background: "none", border: "1px solid #1a2f1a", borderRadius: 8, color: "#4AE54A", fontSize: 13, cursor: "pointer" }}>
          Edit Business Details
        </button>
      </div>

      {!isPro && (
        <div style={{ marginTop: 20, background: "linear-gradient(135deg, #0f2f0f, #1a3f1a)", border: "1px solid #2a5f2a", borderRadius: 12, padding: "18px", textAlign: "center" }}>
          <p style={{ fontSize: 13, color: "#9ad99a", marginBottom: 12 }}>Unlock unlimited replies, daily content, and remove Replo branding</p>
          <button onClick={() => onNavigate("upgrade")} style={{ ...btnPrimary(true), width: "auto", padding: "10px 24px", marginBottom: 0 }}>Go Pro →</button>
        </div>
      )}
    </div>
  );
...
// ─── Reply History ──────────────────────────────────────────────────────────
function ReplyHistory({ isPro, onUpgrade }) {
  const [history, setHistory] = useState(() => store.get("replo_reply_history", []));
  const [copied, setCopied] = useState(null);
  ...
  const clearHistory = () => {
    store.set("replo_reply_history", []);
    setHistory([]);
  };

  if (!isPro && history.length > 5) {
    // free users only see last 5
  }
  ...
  return (
    <div style={sectionWrap}>
      <h2 style={sectionTitle}>▥ Reply History</h2>
      <p style={sectionSub}>Every reply you've generated, saved automatically. {!isPro && <span style={{ color: "#4AE54A" }}>Free users see last 5 — upgrade for full history</span>}</p>

      {history.length === 0 ? (
        <div style={{ padding: "40px 20px", textAlign: "center", color: "#445544" }}>
          <div style={{ fontSize: 32, marginBottom: 12 }}>▥</div>
          <p style={{ fontSize: 14 }}>No replies yet. Generate your first one in Smart Reply.</p>
        </div>
      ) : (
        <>
          {visibleHistory.map((h, i) => (
            <div key={i} style={{ ...resultBox, marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: "#556655", marginBottom: 8, fontFamily: "monospace" }}>
                {new Date(h.timestamp).toLocaleDateString("en-US", { month: "short", day: "numeric" })} · {new Date(h.timestamp).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}
              </div>
              <div style={{ fontSize: 12, color: "#556655", marginBottom: 6 }}>Customer asked:</div>
              <p style={{ fontSize: 13, color: "#9ad99a", fontFamily: "Georgia, serif", marginBottom: 12, fontStyle: "italic" }}>"{h.customerMessage}"</p>
              <div style={{ fontSize: 12, color: "#556655", marginBottom: 6 }}>You replied:</div>
              <p style={{ fontSize: 14, color: "#e8f5e8", lineHeight: 1.7, fontFamily: "Georgia, serif", marginBottom: 14 }}>{h.aiReply}</p>
              <button onClick={() => copy(h.aiReply, i)} style={{ ...btnPrimary(true), width: "auto", padding: "8px 16px", fontSize: 13, marginBottom: 0 }}>
                {copied === i ? "Copied! ✓" : "Copy Again"}
              </button>
            </div>
          ))}
          {!isPro && history.length > 5 && (
            <div style={{ textAlign: "center", padding: "20px", background: "#0d150d", border: "1px dashed #1a2f1a", borderRadius: 12, marginBottom: 16 }}>
              <p style={{ fontSize: 13, color: "#556655", marginBottom: 12 }}>{history.length - 5} more replies hidden. Upgrade to see your full history.</p>
              <button onClick={onUpgrade} style={{ ...btnPrimary(true), width: "auto", padding: "10px 20px", marginBottom: 0 }}>Go Pro →</button>
            </div>
          )}
          <button onClick={clearHistory} style={{ width: "100%", padding: "12px", background: "none", border: "1px solid #1a2f1a", borderRadius: 10, color: "#556655", fontSize: 13, cursor: "pointer" }}>
            Clear History
          </button>
        </>
      )}
...
export default function App() {
  const [screen, setScreen]         = useState(() => store.get("replo_profile", null) ? "app" : "landing"); // landing | login | app
  ...
  const [loginEmail, setLoginEmail] = useState(() => store.get("replo_login_email", ""));

  const handleOnboardDone = (p) => { setProfile(p); setOnboard(false); setScreen("app"); setSection("dashboard"); };
  const handleUpgrade = () => setPaywall(true);
  const handleProSuccess = () => { setIsPro(true); setPaywall(false); };
  const handleNavigate = (id) => { if (id === "upgrade") { setPaywall(true); } else { setSection(id); } };

  const handleLogin = (email) => {
    store.set("replo_login_email", email);
    setLoginEmail(email);
    if (profile) { setScreen("app"); } else { setOnboard(true); setScreen("app"); }
  };
  ...
  const renderSection = () => {
    switch (activeSection) {
      case "dashboard": return <Dashboard profile={profile || {}} isPro={isPro} onNavigate={handleNavigate} onEditBusiness={() => setOnboard(true)} />;
      ...
      case "history":   return <ReplyHistory isPro={isPro} onUpgrade={handleUpgrade} />;
      ...
      default:          return <Dashboard profile={profile || {}} isPro={isPro} onNavigate={handleNavigate} onEditBusiness={() => setOnboard(true)} />;
    }
  };

  const goHome = () => { setScreen(profile ? "app" : "landing"); if (profile) setSection("dashboard"); };
    ...
    <div style={{ minHeight: "100vh", background: "#0a0f0a", color: "#e8f5e8", fontFamily: "'Helvetica Neue', Arial, sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      <nav style={{ padding: "14px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #1a2f1a", position: "sticky", top: 0, background: "#0a0f0a", zIndex: 100 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {!isPro && screen === "app" && <button onClick={() => setPaywall(true)} style={{ background: "#4AE54A", color: "#0a0f0a", border: "none", padding: "7px 14px", borderRadius: 8, fontSize: 13, fontWeight: 700 }}>Go Pro</button>}
          {screen === "landing" && <button onClick={() => setScreen("login")} style={{ background: "none", border: "1px solid #1a2f1a", color: "#e8f5e8", padding: "7px 16px", borderRadius: 8, fontSize: 13, fontWeight: 600 }}>Log In</button>}
        </div>
      </nav>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 24px", animation: "fadeUp 0.6s ease" }}>
            <div style={{ maxWidth: 560, width: "100%", textAlign: "center" }}>
              ...
              <button onClick={() => setScreen("login")} style={{ ...btnPrimary(true), width: "auto", padding: "16px 40px", fontSize: 16 }}>Get Started Free →</button>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 48 }}>
                {SECTIONS.filter(s => s.id !== "dashboard" && s.id !== "history").slice(0, 4).map(s => (
          ...
          <footer style={{ padding: "16px 24px", borderTop: "1px solid #1a2f1a", display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
      ...
      {/* LOGIN SCREEN (login-ready structure, currently just collects email, no password backend yet) */}
      {screen === "login" && (
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 24px", animation: "fadeUp 0.4s ease" }}>
          <div style={{ maxWidth: 380, width: "100%" }}>
            <div style={{ textAlign: "center", marginBottom: 32 }}>
              <div style={{ width: 48, height: 48, borderRadius: "50%", background: "#4AE54A", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 900, color: "#0a0f0a", margin: "0 auto 16px" }}>R</div>
              <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 24, color: "#e8f5e8", marginBottom: 8 }}>Welcome to Replo</h2>
              <p style={{ color: "#556655", fontSize: 14, fontFamily: "Georgia, serif" }}>{profile ? "Log in to continue" : "Enter your email to get started"}</p>
            </div>
            <input
              type="email"
              defaultValue={loginEmail}
              id="replo-login-email-input"
              placeholder="you@example.com"
              style={{ ...inputStyle, marginBottom: 14, textAlign: "center" }}
            />
            <button onClick={() => { const val = document.getElementById("replo-login-email-input").value; if (val.includes("@")) handleLogin(val); }}
              style={{ ...btnPrimary(true), marginBottom: 12 }}>
              Continue →
            </button>
            <button onClick={() => setScreen("landing")} style={{ width: "100%", padding: "12px", background: "none", color: "#556655", border: "1px solid #1a2f1a", borderRadius: 10, fontSize: 14 }}>
              ← Back
            </button>
          </div>
        </div>
